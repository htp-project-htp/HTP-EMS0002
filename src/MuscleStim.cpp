...
#include "MuscleStim.h"

MuscleStim::MuscleStim(uint8_t pin)
...
void MuscleStim::begin() {
    pinMode(_pin, OUTPUT);
    digitalWrite(_pin, LOW);
}

void MuscleStim::setFrequency(uint16_t frequency) {
    if (frequency > 0 && frequency <= 50) {
        _frequency = frequency;
    }
}

void MuscleStim::setDutyCycle(uint8_t duty) {
    if (duty <= 100) {
...
void MuscleStim::setOnOffTimers(unsigned long onTime, unsigned long offTime) {
...
void MuscleStim::stop() {
    _isRunning = false;
    digitalWrite(_pin, LOW);
}


void MuscleStim::update() {
